...
const placeholderCards: Project[] = [
  ...
  ...Array.from({ length: 5 }).map((_, index) => ({
    ...
    description:
      index === 0
        ? 'Онбордированный AI-чат-бот консультирует клиентов по ассортименту, помогает выбрать товар и мягко ведет к покупке, работая на сервере заказчика без внешних API.'
        : index === 1
            ...
            {placeholderCards.map((card, index) => (
              ...
              >
                ...
                <div className="flex min-h-[220px] flex-col px-6 pb-6 pt-5">
                  <p className="mb-4 text-[0.98rem] leading-[1.65] text-slate-600">{card.description}</p>
